...
v2p_bumped _main( v_model I )
{
	...
	// [DA_PORT] Motion vectors. m_VP_nojit and m_WVP_old both already carry this object's world
	// matrix (current and previous respectively), so the local-space vertex feeds straight into them.
	// NB skinned vertices are posed by the CURRENT frame's bones in both cases, so a walking NPC
	// reports the movement of the model as a whole but not of its limbs - that needs the previous
	// frame's bone matrices, which is the next step.
#ifdef DA_VELOCITY
	O.hpos_curr	= mul( m_VP_nojit, w_pos );
#ifdef SKIN_NONE
	O.hpos_old	= mul( m_WVP_old, w_pos );
#else
	O.hpos_old	= mul( m_WVP_old, w_pos ); // [DA_PORT] TEMP: current pose, tests object motion only
	O.hpos_old.x += 0.05 * O.hpos_old.w; // [DA_PORT] TEMP: forced offset, must glow
#endif
#endif
	float2 	tc 	= I.tc;
