...
// [DA_PORT] Bone poses of the previous frame, uploaded by CSkeletonX::_Render alongside the current
// ones. Only declared when motion vectors are on: it doubles this constant array.
#ifdef DA_VELOCITY
	float4 	sbones_array_old	[256-22];
#endif

// [DA_PORT] Result of re-posing the vertex with the previous frame's bones. A shader-global rather
// than a field of v_model on purpose: v_model doubles as the vertex-shader INPUT structure, i.e. it
// describes the vertex buffer layout, and adding a field there asks the engine for an attribute that
// does not exist in the buffer. Initialised so that shaders without skinning still read something
// sane.
#ifdef DA_VELOCITY
static float4 da_skin_P_old = float4(0,0,0,1);
#endif
//}
...
v_model skinning_0	(v_model_skinned_0	v)
{
	...
	o.P 		= u_position	(v.P);
#ifdef DA_VELOCITY
	da_skin_P_old	= o.P;	// rigid: nothing to re-pose
#endif
...
v_model skinning_1 	(v_model_skinned_1	v)
{
	...
	#ifdef DA_VELOCITY
	// [DA_PORT] same vertex, previous frame's bones
...
#ifdef DA_VELOCITY
	da_skin_P_old	= skinning_pos(v.P, m0_old,m1_old,m2_old );
#endif
...
v_model skinning_4 	(v_model_skinned_4	v)
{
	...
	float4  m0_old 	= m[0][0]*w[0];
	float4  m1_old 	= m[0][1]*w[0];
	float4  m2_old 	= m[0][2]*w[0];
		m0_old 	+= m[i][0]*w[i];
		m1_old 	+= m[i][1]*w[i];
		m2_old 	+= m[i][2]*w[i];
#endif
